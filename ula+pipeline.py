class Pipeline:
    def __init__(self, clock):
        self.clock = clock
        self.inst = [None for i in range (5)]
    ...
    def atualizaPipeline(self):
        for i in range (len(self.inst)-1, 0, -1):
            self.inst[i] = self.inst[i - 1]

    def execPipeline(self):
        uc = Uctrl()
        print("|Busca         | {} |".format(self.inst[0]))
        print("|Decodificação | {} |".format(self.inst[1]))
        print("|Busca operando| {} |".format(self.inst[2]))
        print("|Execução      | {} |".format(self.inst[3]))
        print("\nClock= {}".format(self.clock))
        if self.inst[3] != None:
            print("Instrução= {}".format(self.inst[3][0]))
            uc.selecOperacao(self.inst[3])   
        else:
            print("Instrução= {}".format(None))
...
def main():
    ...
    with open("entrada.txt") as file:
        for line in file:
            ...
            vetInstrucao.append(instrucao)
        pipe.inst[count] = vetInstrucao[count]
        count += 1
        pipe.execPipeline()
        while count < len(vetInstrucao) or not pipe.verificaVazio():
            pipe.atualizaPipeline()
            if count < len(vetInstrucao):
                pipe.inst[0] = vetInstrucao[count]
                count += 1
            else:
                pipe.inst[0] = None
            input()
